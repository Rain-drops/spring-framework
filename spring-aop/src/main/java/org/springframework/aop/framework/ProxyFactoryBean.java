...
public class ProxyFactoryBean extends ProxyCreatorSupport
	...
	/**
	 * 初始化 Advisor 链，其中有通过对 IoC 容器的 getBean() 方法的调用来获取配置好的 advisor 通知器
	...
	private synchronized void initializeAdvisorChain() throws AopConfigException, BeansException {
		if (!this.advisorChainInitialized && !ObjectUtils.isEmpty(this.interceptorNames)) {
			...
			// 这里添加了 Advisor 链的调用，下面的 interceptorNames 是在配置文件中 通过 interceptorNames 进行配置的。
			// 由于每一个 Advisor 都是被配置为 bean 的，所以通过遍历 interceptorNames 得到的 name，其实就是 bean 的 id，
			// 通过这个 name（id）我们就可以从 IoC 容器中获取对应的实例化 bean(Advisor)
			for (String name : this.interceptorNames) {
				if (name.endsWith(GLOBAL_SUFFIX)) {
					...
					if (this.singleton || this.beanFactory.isSingleton(name)) {
						...
						// 通过 beanFactory 的 getBean() 方法获取 advisor，[advisor 在文件中配置为 bean]
					...
					// 把从 IoC容器 中获取的 advice 放进 advisors 拦截器链，
